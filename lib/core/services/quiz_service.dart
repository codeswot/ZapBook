...
class QuizService {
  ...
  Future<void> generate({
    ...
    final set = await _generator!(milestoneIdx, wordStart, wordEnd);
    if (set != null) {
      _pool.add(set);
    }
  }
  ...
  void stashMilestone(int milestoneIdx, int wordsRead, int pointsBanked) {
  ...
  void _surfaceNext() {
    ...
    final set = _pool.where((s) => s.milestoneIdx == entry.milestoneIdx).firstOrNull;
    if (set != null) {
      _pool.remove(set);
      _onSurface.add(set);
    }
  ...
  void submitQuiz(int milestoneIdx, List<int> answers, QuizSet set) {
    ...
    final score = set.questions.isEmpty
        ? 0.0
